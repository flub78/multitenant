...
class AttachmentControllerTest extends TenantTestCase {
	...
	public function create_elements(int $number = 1, $argv = []) {
		$elements = [];
		for ($i = 0; $i < $number; $i++) {
			$elements[] = Attachment::factory()->create($argv);
		}
		return $elements;
	}
	 ...
	 */
	public function test_attachment_index_json() {
		...
		foreach (["referenced_table", "referenced_id", "user_id", "filename", "description", "file"] as $field) {
			if ($field != "id")
	...
	public function test_show() {
		...
		foreach (["referenced_table", "referenced_id", "user_id", "filename", "description", "file"] as $field) {
			if ($field != "id")
				$this->assertEquals($attachment1->$field, $json[$field]);
		}
	}
	 ...
	 */
	public function test_attachment_store() {
		...
		foreach (["referenced_table", "referenced_id", "user_id", "filename", "description", "file"] as $field) {
			if (CG::testable($table, $field))
				$this->assertEquals($attachment->$field, $json[$field]);
		}
		...
		$back = Attachment::latest()->first();
		$this->assertNotNull($back);
		foreach (["referenced_table", "referenced_id", "user_id", "filename", "description", "file"] as $field) {
			if (CG::testable($table, $field))
				$this->assertEquals($attachment->$field, $back->$field);
		}
	}
	 ...
	 */
	public function test_attachment_store_incorrect_value() {
	...
	public function test_delete() {
		...
		Attachment::factory()->create();
		Attachment::factory()->create();
		$this->assertEquals(2, Attachment::count(), "Two elements after creation");
		...
		$this->assertEquals(1, Attachment::count(), "One elements after delete");
	}
	...
	public function test_update() {
		...
		$updated = Attachment::findOrFail($id);
	 ...
	 * Test pagination
	 */
	public function test_attachment_pagination() {
		Sanctum::actingAs(User::factory()->create(), ['api-access']);
	 ...
	 */
	public function test_attachment_sorting() {
		Sanctum::actingAs(User::factory()->create(), ['api-access']);
		...
		$first_field = ["referenced_table", "referenced_id", "user_id", "filename", "description", "file"][0];
	 ...
	 *
	 */
	public function ttest_attachment_sorting_on_multiple_columns() {
		Sanctum::actingAs(User::factory()->create(), ['api-access']);
