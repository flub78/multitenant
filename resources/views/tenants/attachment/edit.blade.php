  ...
  <div class="card-body">
    ...
    <form method="post" action="{{ route('attachment.update', $attachment->id ) }}" enctype="multipart/form-data">
      ...
      <div class="form-floating mb-2 border">
        <input type="text" class="form-control" name="referenced_table" value="{{ old("referenced_table", $attachment->referenced_table) }}" readonly />
        <label class="form-label" for="referenced_table">{{__("attachment.referenced_table")}}</label>
      </div>

      <div class="form-floating mb-2 border">
        <input type="text" class="form-control" name="referenced_id" value="{{ old("referenced_id", $attachment->referenced_id) }}" readonly />
        <label class="form-label" for="referenced_id">{{__("attachment.referenced_id")}}</label>
      </div>

      <div class="form-floating mb-2 border">
        <input type="text" class="form-control" name="user_id" value="{{ old("user_id", $attachment->user_id) }}" readonly />
        <label class="form-label" for="user_id">{{__("attachment.user_id")}}</label>
      </div>

      <div class="form-floating mb-2 border">
        <input type="text" class="form-control" name="filename" value="{{ old("filename", $attachment->filename) }}" />
